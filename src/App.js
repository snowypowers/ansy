...
class App extends Component {
  ...
  WalletList(props) {
      ...
      if (p.type === 'NEP2') {
        return (
          <NEP2Wallet
            key={p.address}
            data={p}
            removeCallback={this.removeWallet}
            toggleCallback={this.toggleWallet}
          />
        );
      } else {
        return (
    ...
    return <div className="wallets">{pages}</div>;
  }

  WalletPage(wallets) {
    return (
      <div className="wallet-page" key={wallets[0].key}>
        {wallets}
      </div>
    );
  }
        ...
        <div className="App">
          ...
          <div id="print-foot">
            <div className="third middle">
              <button className="full" type="button" onClick={this.print}>
                {' '}
                Print!
              </button>
            </div>
